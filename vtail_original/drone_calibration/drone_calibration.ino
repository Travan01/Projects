...
void loop(){
  ...
  if(error == 0){
    ...
    Serial.println(F("You can now calibrate the esc's and upload the Tricopter code."));
  }
  else{
   Serial.println(F("The setup is aborted due to an error."));
   Serial.println(F("Check your connections and ensure there are no vibrations"));
  }
  while(1);
}
...
void intro(){
  ...
  delay(1000);
  Serial.println(F(""));
  Serial.println(F("Tricopter Setup Program"));
  Serial.println(F(""));
  Serial.println(F("==================================================="));
  delay(1500);
  Serial.println(F("For codes and design visit our hyperlink"));
  Serial.println(F(""));
