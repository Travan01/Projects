...
int counter_channel_1, counter_channel_2, counter_channel_3, counter_channel_4, counter_channel_5, loop_counter;
...
#define SERVO_PIN 1// use pin 5 for servo control
...
void setup(){
  ...
  DDRD |= B10110000;                                                        //Configure digital poort 4, 5 and 7 as output.DDRD rep pin 76543210 in order
  ...
  for (cal_int = 0; cal_int < 1250 ; cal_int ++){                           //Wait 5 seconds before continuing.
    ...
    PORTD &= B01001111;                                                     //Set digital poort 4, 5, 7 low.
...
a = receiver_input_channel_1;
b = (90 - (ypr[2] * 180/M_PI)/1.0);
c = receiver_input_channel_3;
d = (90 +(ypr[1] * 180/M_PI)/1.0);
e = receiver_input_channel_2;
g = receiver_input_channel_4;
h = (90 - (ypr[0] * 180/M_PI)/1.0);
f = 0;
 Serial.print("throttle-");Serial.print("\t"); Serial.print(b+((c-1100)/20));Serial.print("\t");
 Serial.print("throttle + transmitter");Serial.print("\t"); Serial.print(((a-channel)/8)+90+((c-1100)/20));Serial.print("\t");
 Serial.print("f-");Serial.print("\t");Serial.print(f);Serial.print("\t");
...

{
     ...
     else if (channel2 > e )
     {
        myservo3.write(((e-channel2)/8)+90);
         myservo4.write(((e-channel2)/8)+90);
     }
 ...
 if (c > 1100)
{
    ...
    {
       myservo3.write(((g-channel2)/8)+90);
...
else 
{
  ...
  if(battery_voltage < 1000 && battery_voltage > 600)analogWrite(A3, HIGH);
  ...
  else{ // change 900 to 1000us when escs are misbehaving
    esc_1 = 900;                                                           //If start is not 2 keep a 1000us pulse for ess-1.
    esc_2 = 900;                                                           //If start is not 2 keep a 1000us pulse for ess-2.
    esc_4 = 900;                                                           //If start is not 2 keep a 1000us pulse for ess-4.
  }
  ...
  PORTD |= B10110000;                                                       //Set digital outputs 4,5 and 7 high.
